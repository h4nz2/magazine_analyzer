...
# Process articles
def process_magazine_articles_for_questions(magazine_dir)
  article_files = Dir.glob(File.join(magazine_dir, '*.yaml')).reject { |f| f.include?('summary.yaml') }

  puts "Generating questions for #{article_files.length} articles in #{File.basename(magazine_dir)} using Claude..."

  article_files.each_with_index do |article_file, idx|
    begin
      generator = QuestionGenerator.new(article_file)
...
if ARGV.include?('--help') || ARGV.include?('-h')
  puts "Usage: ruby generate_questions.rb [magazine_directory]"
  puts ""
  puts "Defaults: ruby generate_questions.rb all"
  puts ""
...
if magazine_arg == 'all'
  ...
  magazine_dirs.each do |dir|
    process_magazine_articles_for_questions(dir)
    puts "---"
  end
else
  ...
  process_magazine_articles_for_questions(magazine_arg)
end
